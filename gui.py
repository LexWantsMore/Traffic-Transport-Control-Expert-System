...
class Application(tk.Frame):
    ...
    def create_widgets(self, master):
        ...
        self.checkbuttonSafeDistance = tk.Checkbutton(
            self, text='100 meters from the vehicle in front', variable=self.safety)
        ...
        self.radiobuttonMiddleLane = tk.Radiobutton(
            self, text='Middle Lane', variable=self.lane, value='Middle lane')
        self.radiobuttonRightLane = tk.Radiobutton(
            self, text='Right Lane', variable=self.lane, value='Right lane')
        self.labelVisibility = tk.Label(self, text='Visibility:')
        self.radiobuttonNoneVisibility = tk.Radiobutton(
            self, text='None', variable=self.visibility)
        ...
        self.buttonRun = tk.Button(self, text='Monitoring...', command=self.run)
        self.labelOutput = tk.Label(self, text='End of Monitoring.', justify='left')
    ...
    def run(self):
        ...
        if len(Advice) > 0:
            ...
            if 'Leave ASAP' in Advice:
                t += 'Get off the highway as soon as possible at the nearest exit'
...
if __name__ == '__main__':
    app = Application()
    app.master.title('Expressway Speed Monitoring Expert System')
    app.mainloop()
